...
class IsaacPickPlaceEnv:
    ...
    def _build_scene(self):
        ...
        tp, to = np.array([0.0, -0.25, 3]), np.array([0.5, 0.5, 0.5, -0.5])
        # Side camera: positioned at +X, looking toward origin, properly oriented
        sp, so = np.array([2.5, -0.25, 0.25]), np.array([0.0, 0.0, 0.0, 1.0])
    ...
    def step(self, action, render=None):
        ...
        if self._step_counter % 30 == 0:
            print(f"[GRASP] G:{int(is_grasped)} C:{int(c)} S:{int(s)} L:{int(l)}")
    ...
    def _apply_gripper_friction(self):
        try:
            ...
            # Also apply contact report API to the cube
